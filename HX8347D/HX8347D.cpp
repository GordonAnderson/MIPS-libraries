...
#define CS_IDLE       { digitalWrite(_lcd_pin_cs, HIGH); }
#define CS_ACTIVE     { digitalWrite(_lcd_pin_cs, LOW); }
#define CD_DATA       { digitalWrite(_lcd_pin_rs,HIGH); }
#define CD_COMMAND    { digitalWrite(_lcd_pin_rs,LOW); }
#define RESET_IDLE    { digitalWrite(_lcd_pin_reset,HIGH); }
#define RESET_ACTIVE  { digitalWrite(_lcd_pin_reset,LOW); }


#define xchg8(x)     spiwrite((uint8_t)x)
#define write8(x)    { spiwrite((uint8_t)x); }
#define write16(x)   { uint8_t h = (x)>>8, l = x; write8(h); write8(l); }
#define WriteCmd(x)  { CD_COMMAND; write8(x); }
#define WriteData(x) { CD_DATA; write8(x); }

inline uint8_t HX8347::spiwrite(uint8_t c) {
...
HX8347::HX8347(uint8_t CS, uint8_t RS, uint8_t RST)
    : _lcd_pin_rs(RS), _lcd_pin_cs(CS), _lcd_pin_reset(RST), Adafruit_GFX(240, 320)
{
    pinMode(_lcd_pin_rs,OUTPUT);
    pinMode(_lcd_pin_cs,OUTPUT);
    pinMode(_lcd_pin_reset,OUTPUT);
    CS_IDLE;
    RESET_IDLE;
}

void HX8347::disableDisplay(bool state)
{

}

void HX8347::pushColor(uint16_t color)
{
}
...
HX8347::HX8347_kbv():Adafruit_GFX(240, 320)
{
    INIT();
    CS_IDLE;
    RESET_IDLE;
...
void HX8347::WriteCmdData(uint16_t cmd, uint16_t dat)
{
    CS_ACTIVE;
    WriteCmd(cmd);
    WriteData(dat);
...
uint16_t HX8347::readReg(uint16_t reg)
{
    uint8_t h, l;
    CS_ACTIVE;
    WriteCmd(reg);
...
int16_t HX8347::readGRAM(int16_t x, int16_t y, uint16_t * block, int16_t w, int16_t h)
{
...
void HX8347::setRotation(uint8_t r)
{
    uint16_t mac = 0x0800;
...
void HX8347::drawPixel(int16_t x, int16_t y, uint16_t color)
{
...
void HX8347::setAddrWindow(int16_t x, int16_t y, int16_t x1, int16_t y1)
{
    CS_ACTIVE; 
    WriteCmdDataPair(HX8347G_COLADDRSTART_HI, x);
...
void HX8347::fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
    int16_t end;
    if (w < 0) { w = -w; x -= w; }   //+ve w
    end = x + w;
...
void HX8347::pushColors(uint16_t * block, int16_t n, bool first)
{
    uint16_t color;
    CS_ACTIVE;
    if (first) {
...
void HX8347::pushColors(uint8_t * block, int16_t n, bool first)
{
    uint16_t color;
	uint8_t h, l;
    CS_ACTIVE;
...
void HX8347::pushColors(const uint8_t * block, int16_t n, bool first, bool bigend)
{
    uint16_t color;
	uint8_t h, l;
	CS_ACTIVE;
...
void HX8347::invertDisplay(bool i)
{
    uint8_t val = 0x00;
...
void HX8347::vertScroll(int16_t top, int16_t scrollines, int16_t offset)
{
