    ...
    def upload_one_file(self, fd):
        ...
        while True:
            ...
            if chr(command) == 'S':
                ...
                self._serial.write(0xff * (num_bytes - len(xsvf_data)))
                if self._args.debug > 1:
                ...
                if self._args.debug > 0:
                    self.print_lf()
                    print('Device:', argument.decode('utf-8'))
            elif chr(command) == '!':
                if self._args.debug > 0:
                    self.print_lf()
                    print('IMPORTANT:', argument.decode('utf-8'))
            else:
                self.print_lf()
                print('Unrecognized line:',\
                    line.translate(Uploader._translate_str.encode()))
